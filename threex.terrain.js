 ...
 */
THREEx.Terrain.simplexHeightMap	= function(heightMap,xx,zz){
	// get heightMap dimensions
	var width	= heightMap.length
	var depth	= heightMap[0].length
	if (simplex==null) {
		var p = [165,56,250,33,2,147,169,187,198,22,20,56,47,251,235,44,116,241,32,23,68,195,247,113,3,217,192,90,237,20,129,116,250,209,43,205,46,66,30,118,200,23,82,122,129,106,87,207,132,224,168,244,79,93,124,156,159,125,241,48,17,140,255,238,244,62,103,218,38,135,220,217,34,59,218,55,193,43,150,36,164,169,41,106,72,60,165,82,76,73,132,217,205,255,85,10,106,253,157,59,220,101,216,93,56,229,179,74,3,109,248,139,188,160,177,64,10,129,176,235,204,143,118,79,223,165,74,164,95,121,147,83,180,107,42,220,192,189,249,44,99,71,81,11,132,254,215,109,228,212,18,28,230,191,203,80,181,228,3,85,12,220,64,7,211,7,252,86,166,72,236,71,251,169,27,158,218,30,92,54,193,176,118,192,83,139,251,147,234,138,141,173,105,130,60,58,241,187,133,173,93,152,138,177,34,123,171,160,224,237,72,129,2,77,25,193,178,209,10,252,135,112,83,54,112,79,116,229,20,68,232,73,199,129,153,193,149,13,119,51,86,54,158,159,149,117,138,133,108,198,38,202,174,39,253,69];
		simplex	= new SimplexNoise(Math,p)
	}

	for(var x = xx; x < width+xx; x++){
		for(var z = zz; z < depth+zz; z++){
			// compute the height
			var height	= 0
			var level	= 8
...
THREEx.Terrain.heightMapToCanvas	= function(heightMap, canvas){
	...
	for(var x = 0; x < canvas.width; x++){
		for(var y = 0; y < canvas.height; y++){
			...
			var color	= THREEx.Terrain.heightToColor(height/10)
			//console.log(color)
